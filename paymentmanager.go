...
func (p *PaymentManager) handleCmd(cmd interface{}) {
	...
	case *sendPaymentCmd:
		...
		peerLog.Info("Before: p.chPaymentOut <- paymentPkt")
		p.chPaymentOut <- paymentPkt
		peerLog.Info("After: p.chPaymentOut <- paymentPkt")
		cmd.err <- nil
	default:
...
func (p *PaymentManager) handlePaymentPacket(paymentPkt *PaymentPacket) {
	...
	case *lnwire.PaymentInitiation:
		...
		// TODO(mkl): validate reverse onion path
	...
	case *lnwire.PaymentInitiationConfirmation:
		...
		peerLog.Infof("Before sending HTLCAddRequest: %v", htlcAdd)
		htlcPkt := &htlcPacket{
